    ...
    <    >
      ...
      <Section>
        ...
        <Card className="max-w-2xl mx-auto">
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              <label className="text-[11px] uppercase tracking-[0.15em] text-foreground/50 font-bold">Region</label>
              <select value={region} onChange={(e) => setRegion(e.target.value)} className="mt-2 w-full bg-black/40 border border-white/[0.08] rounded-lg px-3 py-2 text-white outline-none focus:border-primary/40">{Object.keys(rates).map((r) => <option key={r}>{r}</option>)}</select>
            </div>
            <div>
              <label className="text-[11px] uppercase tracking-[0.15em] text-foreground/50 font-bold">Role</label>
              <select value={role} onChange={(e) => setRole(e.target.value)} className="mt-2 w-full bg-black/40 border border-white/[0.08] rounded-lg px-3 py-2 text-white outline-none focus:border-primary/40">{Object.keys(rates[region]).map((r) => <option key={r}>{r}</option>)}</select>
            </div>
            <div>
              <label className="text-[11px] uppercase tracking-[0.15em] text-foreground/50 font-bold">Hours / mo</label>
              <input type="number" value={hours} onChange={(e) => setHours(+e.target.value || 0)} className="mt-2 w-full bg-black/40 border border-white/[0.08] rounded-lg px-3 py-2 text-white outline-none focus:border-primary/40" />
            </div>
          </div>
          <div className="mt-6 pt-6 border-t border-white/[0.07] flex items-end justify-between">
            <div>
              <div className="text-[11px] uppercase tracking-[0.2em] text-primary font-bold">Estimated monthly</div>
              <div className="mt-1 text-4xl font-extrabold text-white">${monthly.toLocaleString()}</div>
            </div>
      ...
      <Section>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          ...
          ].map((c) => <Card key={c.t}><div className="text-white font-medium mb-2">{c.t}</div><div className="text-foreground/60 text-sm">{c.d}</div></Card>)}
        </div>
