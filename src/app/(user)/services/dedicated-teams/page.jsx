...
export default function Page() {
  return (
   <>
      <div className=" px-6 max-w-7xl mx-auto"><Breadcrumb items={[{ label: "Home", to: "/" }, { label: "Services" }, { label: "Dedicated Teams" }]} /></div>
      <PageHero
        eyebrow="Delivery Model"
        title={<>Dedicated <span className="text-primary">Expert Teams</span></>}
        sub="A full-time developer assigned solely to your project — embedded in your workflow, owning outcomes end-to-end."
      />
      <Section>
      ...
      <Section>
        ...
        <div className="max-w-2xl mx-auto"><Card className="p-10">
          <div className="text-[11px] uppercase tracking-[0.2em] text-primary font-bold">Dedicated Engineer</div>
          <div className="mt-3 flex items-end gap-2"><div className="text-5xl font-extrabold text-white">$4,800</div><div className="text-foreground/50 pb-2">/ month</div></div>
          <div className="mt-2 text-foreground/55 text-sm">Senior-level · 160 hrs/mo · 30-day exit</div>
          <ul className="mt-6 space-y-3">
            {["Full-time engineer dedicated to one client", "Replacement guarantee within 7 days", "Dedicated success manager + weekly QBR", "All IP & code ownership transferred to you", "No recruiter or platform fees"].map((f) => (
              <li key={f} className="flex items-start gap-2 text-sm text-foreground/80"><Check className="size-4 text-primary mt-0.5 flex-shrink-0" />{f}</li>
            ))}
          </ul>
          <Link href="/contact" className="mt-8 inline-flex items-center gap-2 bg-white text-black font-bold px-5 py-3 rounded-full text-sm">Talk to Expert <ArrowRight className="size-4" /></Link>
        </Card></div>
      </Section>
      <Section>
